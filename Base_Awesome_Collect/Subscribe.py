...
async def wallabagAPI(loop):
    ...
    async def create_entry(title, url, tags):
        ...
        try:
            ...
            tag_flag = 0
            publishers_flag = 0
            entry = Entry.create(id=data["id"], user_name=data["user_name"], title=data["title"], url=data["url"],
            ...
            if data["published_by"]:
                for row in data["published_by"]:
                    if len(row) != 0:
                        publishers_flag = 1
                        publishers = Published_by.create(publisher=row, title=data["title"], id=data["id"],
                                                         url=data["url"])
